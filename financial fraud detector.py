...
# ---------------------------------------------------------
# Behavioral Node Features (NEW)
# ---------------------------------------------------------

# Transaction frequency per account
tx_freq = df.groupby("nameOrig").size()
tx_freq = tx_freq.reindex(nodes, fill_value=0)

# Average transaction amount
avg_amount = df.groupby("nameOrig")["amount"].mean()
avg_amount = avg_amount.reindex(nodes, fill_value=0)

# Balance change
df["balance_change"] = df["oldbalanceOrg"] - df["newbalanceOrig"]
avg_balance_change = df.groupby("nameOrig")["balance_change"].mean()
avg_balance_change = avg_balance_change.reindex(nodes, fill_value=0)

# ---------------------------------------------------------
# Node features (Graph + Behavioral)
# ---------------------------------------------------------
deg_out = degree(data.edge_index[0], num_nodes=data.num_nodes)
deg_in = degree(data.edge_index[1], num_nodes=data.num_nodes)

tx_freq_tensor = torch.tensor(tx_freq.values, dtype=torch.float)
avg_amount_tensor = torch.tensor(avg_amount.values, dtype=torch.float)
balance_change_tensor = torch.tensor(avg_balance_change.values, dtype=torch.float)

data.x = torch.log1p(torch.stack([
    deg_out,
    deg_in,
    tx_freq_tensor,
    avg_amount_tensor,
    balance_change_tensor
], dim=1))
...
model = GraphSAGE(in_channels=5, hidden_channels=64, out_channels=2).to(device)
